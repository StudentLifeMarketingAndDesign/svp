<?php
class NewEpisodePage extends Page {
	...
	function PreviousEpisodes() {
		...
		return DataObject::get("EpisodePage", "ParentID = $Parent->ID AND Airdate < $Airdate", Null, Null, 2);
	}
	...
	function FollowingEpisodes($amt = 2) {
		$Parent = $this->getParent();
		$Airdate = mysql_real_escape_string($this->Airdate);
		return DataObject::get("EpisodePage", "ParentID = $Parent->ID AND Airdate >= $Airdate", Null, Null, $amt);
	}
	function ShownEpisodes() {
		$previous = $this->PreviousEpisodes();
		$following = $this->FollowingEpisodes(5 - count($previous));
...
class NewEpisodePage_Controller extends Page_Controller {
